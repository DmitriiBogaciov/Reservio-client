...
//Props
import { ListItemProps } from "@/props/ListItem.props";
                        ...
                        <ul className="p-0">
                            {
                                listItems.map((item, i) => {
                                    return (
                                        <li key={i} onClick={() => { (item.onClick) ? item.onClick(item.value) : "" }} className={`text-left p-2 ${((i + 1) % 2 === 0) ? "bg-slate-200" : "bg-slate-400"} ${(item.onClick) ? "cursor-pointer hover:scale-105 transition-[200ms]" : ""}`}>
                                            {item.label}
                                        </li>
                                    )
                                })
